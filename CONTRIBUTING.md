...
通过issue进行贡献非常简单，但是也有一些规则:

* 将issue标签页仅用于讨论为指南的问题**禁止个人hackintosh问题**。这不是一个用来讨论安装问题的地方。
...
* 动动你的脑筋(求你了)。
* 请仔细检查你提交的PR。
* 如果我们认为您的PR不符合要求或包含模糊的信息，我们可能会予以拒绝。不过，我们通常会告诉您被拒的原因，或要求您进行修改。
  * 对于较大的提交，若能提供来源信息，我们将非常感谢，应为这有助于我们核实您提供的消息是否属实。
* 图片必须托管在 仓库本地的`../images/` 文件夹下。
* 您的PR必须通过 Markdown 语法检查，且所有问题均已解决。
...
* 检查格式和拼写:
  * `npm test`
  * `npm run lint` and `npm run spellcheck` (分别检查)
...
* [Potato-is-rotten](https://github.com/Potato-is-rotten/OpenCore-Install-Guide)(中文)
